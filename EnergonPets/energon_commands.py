...
# Use the global user_data_manager instance
...
class EnergonCommands(commands.Cog):
    ...
    def create_game_info_embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            ...
            description="Experience the ultimate Cybertronian energy collection adventure with multiple game modes and features!",
            color=0x00ff00
        )
        embed.add_field(
            name="🎯 Core Objective",
            value=f"Collect **{WIN_CONDITION:,} Energon** to win rounds and climb the global leaderboards!",
            inline=False
        )
        embed.add_field(
            name="⚡ Energon Collection",
            value="`/scout` - Low-risk Energon search (safer option)\n`/search` - High-risk, high-reward exploration\n`/energon_stats` - View stats, rankings, and leaderboards",
            inline=False
        )
        embed.add_field(
            name="🎰 Slot Machine Casino",
            value="`/slots` - Play the Energon slot machine with multiple themes:\n• **Easy** - Skills Theme (1:80 odds)\n• **Medium** - Characters Theme (1:512 odds)\n• **Hard** - Zodiac Theme (1:1728 odds)\n\nChoose between Fun mode (no risk) or Bet mode (risk Energon for bigger rewards)!",
            inline=False
        )
        embed.add_field(
            name="💰 CyberCoin Market",
            value="`/cybercoin_market` - Trade in the dynamic cryptocurrency market\n`/cybercoin_profile` - View your portfolio and transaction history\n\nInvest your Energon in CyberCoin with real-time price fluctuations, market events, and holiday effects!",
            inline=False
        )
        embed.add_field(
            name="🏆 Advanced Features",
            value="• **Global Banking System** - Persistent Energon storage across rounds\n• **Cross-Channel Gameplay** - Compete with players across all servers\n• **Pet Integration** - Your EnergonPets provide search bonuses\n• **Comprehensive Statistics** - Track wins, losses, and total earnings\n• **Dynamic Market Events** - Real-time events affect CyberCoin prices",
            inline=False
        )
        embed.add_field(
            name="📊 Progression & Rewards",
            value="Earn Energon through various activities, bank it for safety, invest in CyberCoin for potential profits, and climb the global leaderboards. Your EnergonPets enhance your search capabilities!",
            inline=False
        )
        embed.set_footer(text="🤖 Transform and roll out to collect Energon across Cybertron!")
        return embed
    ...
    async def scout(self, ctx: commands.Context) -> None:
        ...
        try:
            ...
            channel_id = int(ctx.channel.id)  # Use int for consistency
            ...
            if game_active or energon_data.get('in_energon_rush', False):
                ...
                if not energon_data.get('in_energon_rush', False):
                    success = await self._add_player_to_game(channel_id, player_id)
                    if success:
                ...
                success = await self._start_new_game(channel_id, player_id)
                if success:
                    energon_data['in_energon_rush'] = True
    ...
    async def _calculate_pet_bonus(self, player_id: int) -> tuple[int, bool]:
        ...
        try:
            ...
            if pet_data:
                ...
                pet_bonus = min(5 + (pet_level // 50) * 5, 50)  # Starts at 5%, increases 5% per 50 levels, max 50% at level 500
                has_pet = True
        except Exception as e:
    ...
    async def _process_search_outcome(self, ctx: commands.Context, player_id: str, pet_bonus: int, has_pet: bool, current_energon: int) -> int:
        ...
        if has_pet:
            ...
            if pet_data:
                ...
                leveled_up, level_gains = await add_experience(ctx.author.id, search_xp, "search")
                ...
                if leveled_up and level_gains:
                    try:
                        level_up_embed = await create_level_up_embed(pet_data, level_gains, ctx.author.id)
        ...
        # Track pet search statistics if pet is present
        if has_pet and outcome_energon != 0:
            try:
                energon_data = await user_data_manager.get_energon_data(player_id)
                
                # Increment pet search helps counter
                energon_data['pet_search_helps'] = energon_data.get('pet_search_helps', 0) + 1
                
                # Track energon won when pet is present (only positive amounts)
                if outcome_energon > 0:
                    energon_data['pet_search_energon'] = energon_data.get('pet_search_energon', 0) + outcome_energon
                
                await user_data_manager.save_energon_data(player_id, energon_data)
            except Exception as e:
                print(f"Error tracking pet search statistics: {e}")
    ...
    async def slots_command(self, ctx: commands.Context, mode: str = "fun", 
        ...
        try:
            ...
            if mode == "bet":
                ...
                success, message = await self.game_manager.use_energon_for_bet(str(ctx.author.id), bet)
                if not success:
    ...
    async def _start_new_game(self, channel_id: int, starter_player_id: str) -> bool:
        ...
        return await self.game_manager.start_new_game(channel_id, starter_player_id)

    async def _add_player_to_game(self, channel_id: int, player_id: str) -> bool:
        """Add a player to the active game."""
        return await self.game_manager.add_player_to_game(channel_id, player_id)

    async def _end_game(self, channel_id: int, ctx) -> None:
    ...
    async def energon_stats(self, ctx: commands.Context) -> None:
        ...
        embed.add_field(name="💰 Energon Statistics", 
                       value=f"**Total Earned:** {stats['total_earned']:,}\n**Total Spent:** {stats['total_spent']:,}\n**Net:** {stats['total_earned'] - stats['total_spent']:,}", 
                       inline=True)
        embed.add_field(name="🏦 Bank & Stats", 
        ...
        if leaderboard:
            medals = ["🥇", "🥈", "🥉"] + ["🔸"] * 7
            leaderboard_text = [
                f"{medals[i]} **{i+1}.** {entry['username']}: {entry['total_energon']:,} Energon"
