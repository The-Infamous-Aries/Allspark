    ...
    async def show_results(self, slot_emojis: List[str]):
        ...
        if self.mode == "bet":
            try:
                ...
                # Update main energon balance in user data - use proper energon tracking
                if winnings > 0:
                    await user_data_manager.add_energon(str(self.user.id), winnings, "slots")
                else:
                    await user_data_manager.subtract_energon(str(self.user.id), self.bet, "slots")
