...
class PetStatusView(discord.ui.View):
    ...
    async def _add_embed_fields(self, embed: discord.Embed, pet: Dict[str, Any], 
        ...
        try:
            user_data = await user_data_manager.get_user_data(user_id, str(user_id))
            ...
            if combiner_data:
                ...
                if team_id:
                    ...
                    for emoji, part_name in part_names.items():
                        members = team_data.get(emoji, [])
                        for member_entry in members:
                            try:
                                member_id = member_entry.get("user_id")
                                ...
                                if member_pet:
                                    ...
                                    team_members.append(f"{emoji} **{pet_name}** ({display_name}) - **{member_role}**")
                                else:
                                    team_members.append(f"{emoji} Unknown Pet - **{member_role}**")
                            except Exception as e:
                                logger.error(f"Error getting member data for {member_entry}: {e}")
                                team_members.append(f"{emoji} Unknown Pet - **Unknown Role**")
...
class PetEquipmentView(discord.ui.View):
    ...
    def __init__(self, user_id: int, pet_system, items: List[Dict[str, Any]], equipment_data: Dict[str, Dict[str, Any]] = None):
        super().__init__(timeout=300)  # 5 minute timeout
        self.user_id = user_id
        self.pet_system = pet_system
        self.items = items
        ...
        self._filtered_items = items  # Filtered items for display
        self._current_filter = "all"  # Current filter type
    ...
    def _get_page_items(self, page: int) -> List[Dict[str, Any]]:
        ...
        return self._filtered_items[start_idx:end_idx]
    
    def _enrich_item_with_equipment_data(self, item: Dict[str, Any]) -> Dict[str, Any]:
        """Enrich item with full equipment data from equipment JSON"""
        item_id = item.get('id')
        if not item_id or not self.equipment_data:
            return item
        
        # Look up equipment data by ID - need to search through nested structure
        equipment_info = self._find_equipment_by_id(item_id)
        if equipment_info:
            # Merge equipment data with item data, prioritizing equipment data
            enriched_item = item.copy()
            enriched_item.update({
                'name': equipment_info.get('name', item.get('name', 'Unknown Item')),
                'description': equipment_info.get('description', item.get('description', 'No description available')),
                'rarity': equipment_info.get('rarity', item.get('rarity', 'common')),
                'type': equipment_info.get('type', item.get('type', 'unknown')),
                'stat_bonus': equipment_info.get('stat_bonus', item.get('stat_bonus', {}))
            })
            return enriched_item
        
        return item
    
    def _find_equipment_by_id(self, item_id: str) -> Dict[str, Any]:
        """Find equipment by ID in the nested equipment data structure"""
        if not self.equipment_data:
            return {}
        
        # Define equipment categories to search through
        categories = ['chassis_plating', 'energy_cores', 'utility_modules']
        
        for category in categories:
            category_data = self.equipment_data.get(category, {})
            equipment_data = category_data.get('equipment', {})
            
            # Search through each rarity level
            for rarity_data in equipment_data.values():
                if isinstance(rarity_data, dict) and item_id in rarity_data:
                    return rarity_data[item_id]
        
        return {}
    
    def _filter_items_by_type(self, filter_type: str) -> None:
        ...
        # Update pagination
        self.total_pages = max(1, (len(self._filtered_items) + self.ITEMS_PER_PAGE - 1) // self.ITEMS_PER_PAGE)
        self.current_page = 0  # Reset to first page
        self._cached_embeds.clear()  # Clear cache
    ...
    async def _build_embed(self, page: int) -> discord.Embed:
        ...
        if self._current_filter != "all":
            filter_emoji = self.TYPE_EMOJIS.get(self._current_filter, '📦')
            filter_text = f" • {filter_emoji} {self._current_filter.replace('_', ' ').title()}"
        
        embed = discord.Embed(
            title=f"🎒 {pet_name}'s Equipment Collection",
            description=f"Page {page + 1}/{self.total_pages} • {len(self._filtered_items)} items{filter_text}",
        ...
        for idx, item in enumerate(page_items, start=page * self.ITEMS_PER_PAGE):
            embed.add_field(**self._format_item_field(item, idx))
            
        return embed
    
    def _format_item_field(self, item: Dict[str, Any], idx: int) -> Dict[str, str]:
        """Format an item as an embed field"""
        # Enrich item with equipment data if available
        enriched_item = self._enrich_item_with_equipment_data(item)
        
        rarity = enriched_item.get('rarity', 'common').lower()
        item_type = enriched_item.get('type', 'unknown')
        name = enriched_item.get('name', 'Unknown Item')
        description = enriched_item.get('description', 'No description available')
        
        # Add equipped indicator if item is equipped
        equipped_indicator = " ✅" if item.get('equipped') else ""
        
        return {
            'name': f"{self.RARITY_EMOJIS.get(rarity, '⚪')} {self.TYPE_EMOJIS.get(item_type, '📦')} {name}{equipped_indicator}",
            'value': self._format_item_value(description, enriched_item.get('stat_bonus', {})),
            'inline': False
        }
...
class EquipmentTypeSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Apply filter
        view._filter_items_by_type(filter_type)
        view._current_filter = filter_type
...
class PetSystem:
    ...
    def _get_faction_stats(self, faction: str) -> Dict[str, int]:
        ...
        if faction == 'autobot':
            return {"attack": 7, "defense": 10, "happiness": 200, "energy": 100}
        return {"attack": 10, "defense": 7, "happiness": 100, "energy": 200}
    ...
    async def add_experience(self, user_id: int, xp_amount: int, source: str = "unknown", equipment_stats: Optional[Dict[str, int]] = None) -> Tuple[bool, Optional[Dict[str, Any]]]:
    ...
    async def equip_item(self, user_id: int, item_id: str) -> Tuple[bool, str]:
        ...
        item_data = self._find_item_in_equipment_data(equipment_data, item_id)
        
        if not item_data:
            # Check if it's a category name and provide helpful error
            category_names = ['chassis plating', 'energy cores', 'utility modules']
            if item_id.lower() in category_names:
                return False, f"'{item_id}' is a category name, not a specific item. Please select a specific item from the autocomplete suggestions."
            else:
                return False, f"Invalid item ID: '{item_id}'. Please select from your available items using the autocomplete suggestions."
    ...
    def _find_item_in_equipment_data(self, equipment_data: Dict[str, Any], item_id: str) -> Optional[Dict[str, Any]]:
        """Find an item in the nested equipment data structure"""
        # Search through all equipment categories
        for category in ['chassis_plating', 'energy_cores', 'utility_modules']:
            category_data = equipment_data.get(category, {})
            if isinstance(category_data, dict):
                equipment_section = category_data.get('equipment', {})
                if isinstance(equipment_section, dict):
                    ...
                    for rarity in ['common', 'uncommon', 'rare', 'epic', 'legendary', 'mythic']:
                        ...
                        if isinstance(rarity_items, dict) and item_id in rarity_items:
                            return rarity_items[item_id]
                    
                    # If not found in rarities, try direct lookup in equipment_section
                    if item_id in equipment_section:
                        return equipment_section[item_id]
    ...
    async def battle_pet(self, user_id: int, opponent_id: int) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            if battle_result['success']:
                ...
                equipment_stats = await self.get_equipment_stats(user_id)
                leveled_up, level_gains = await add_experience(user_id, xp_gain, "battle", equipment_stats)
