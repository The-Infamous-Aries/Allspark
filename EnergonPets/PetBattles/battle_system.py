        ...
        try:
            ...
            for user_id, data in self.player_data.items():
                if data['alive'] and data['hp'] > 0:
                    ...
                    level_up_result = await add_experience(str(user_id), total_reward, "battle")
                    ...
                    if level_up_result and level_up_result.get('leveled_up'):
                        ...
                        survivor_updates[str(user_id)]['level_up'] = level_up_result
                    ...
                    if looted_equipment:
                        # Store the first item for display (or could be modified to show all)
                        self.rewards['equipment_loot'][user_id] = looted_equipment[0]
            ...
            for user_id, survivor_data in survivor_updates.items():
                ...
                if 'level_up' in survivor_data:
                    level_up_result = survivor_data['level_up']
                    if level_up_result.get('embed'):
                        try:
                            await self.ctx.channel.send(embed=level_up_result['embed'])
                        except Exception as e:
...
class EphemeralActionView(discord.ui.View):
    ...
    async def handle_action_selection(self, interaction: discord.Interaction, action: str):
        ...
        try:
            ...
            # Other error, try to disable the view instead
            try:
                await interaction.message.edit(view=None, delete_after=1)
            except discord.NotFound:
                # Message was deleted while we were trying to edit it
                pass
            except Exception:
