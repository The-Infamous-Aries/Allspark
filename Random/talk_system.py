        ...
        try:
            async with ctx.typing():
                ...
                def is_content_clean(text):
                    ...
                    # Blacklist of inappropriate words/phrases
                    blacklist = [
                        'kill', 'die', 'death', 'suicide', 'murder', 'rape', 'sex', 'porn', 'nude', 'naked',
                        'f*ck', 'shit', 'damn', 'hell', 'bitch', 'ass', 'bastard', 'crap', 'piss',
                        'hate', 'racist', 'nazi', 'terrorist', 'violence', 'blood', 'gore'
                    ]
                    ...
                    try:
                        ...
                        if roasts_data:
                            ...
                            for _ in range(5):
                                ...
                                if is_content_clean(chosen_roast):
                                    return {
                                        'roast': chosen_roast,
                                        'source': f'Clean Roast Database ({chosen_category.replace("_", " ").title()})',
                                        'category': chosen_category
                                    }
                            
                            # If we couldn't find a clean roast after 5 tries, return None
                            print("Warning: Could not find clean roast after 5 attempts")
                ...
                # Use Transformers fallback if local database failed
                if not roast_data:
                    roast_data = {
                ...
                embed.add_field(
                    name="📂 Roast Category",
                    value=f"`{roast_data.get('category', 'general').replace('_', ' ').title()}`",
                    inline=True
                )
