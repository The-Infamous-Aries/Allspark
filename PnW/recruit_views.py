...
class PaginatedRecruitmentView(View):
    ...
    async def recruit_today(self, interaction: discord.Interaction, button: Button):
        ...
        # Close the current embed and start synchronous recruitment
        try:
            ...
            embed.add_field(
                name="📊 Status", 
                value="Preparing to send recruitment messages...", 
                inline=False
            )
            ...
            # Start synchronous recruitment with progress updates
            results = await self.recruit_cog.send_recruitment_messages(
                today_nations, 
                interaction.user.id,
    ...
    async def recruit_last_week(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            embed.add_field(
                name="⚡ Estimated Time",
                value=f"Approximately {len(week_nations) * 2.5 / 60:.1f} minutes\n"
    ...
    async def recruit_two_weeks(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            embed.add_field(
                name="⚡ Estimated Time",
                value=f"Approximately {len(two_weeks_nations) * 2.5 / 60:.1f} minutes\n"
                      f"(2-3 seconds per nation to prevent API spam)",
                inline=False
            )
...
class ActivityFilterView(View):
    ...
    async def filter_today(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            # Fetch all nations
            all_nations = await self.recruit_cog.get_all_filtered_nations()
