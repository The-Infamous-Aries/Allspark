...
class CompareCog(commands.Cog):
    ...
    async def compare(self, interaction: discord.Interaction, home: str, away: str):
        try:
            ...
            if not valid_home:
                await interaction.followup.send("❌ Could not resolve any Home alliances. Provide IDs, links, or known names.")
                return
            ...
            if self.query_instance and hasattr(self.query_instance, 'get_alliances_nations_batched'):
                try:
                    ...
                    await interaction.followup.send(f"❌ Failed to fetch Home party data: {e}")
                    return
            else:
                await interaction.followup.send("❌ Query system unavailable for batched Home fetch.")
                return
            ...
            if self.query_instance and hasattr(self.query_instance, 'get_alliances_nations_batched'):
                try:
                    ...
                    await interaction.followup.send(f"❌ Failed to fetch Away party data: {e}")
                    return
            else:
                await interaction.followup.send("❌ Query system unavailable for batched Away fetch.")
                return
