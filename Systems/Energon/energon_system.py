...
try:
    ...
    sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'Allsparks Call'))
    from Allsparks_Call.cyberchronicles import get_rpg_system, add_energon_to_character
except ImportError:
    # Create mock functions if imports fail
    def get_rpg_system():
        return None
    
    def add_energon_to_character(character_name: str, energon_amount: int) -> bool:
        return True
...
GAME_STATE_FILE = os.path.join(JSON_DIR, "game_state.json")
...
class EnergonGameManager:
    ...
    def load_game_state(self) -> None:
        """Load all Energon game state from JSON file."""
        try:
            if os.path.exists(GAME_STATE_FILE):
                ...
                print("No existing game state file found. Initialized empty game state.")
    ...
    def save_game_state(self) -> None:
        """Save all Energon game state to JSON file."""
        self._ensure_state_loaded()
            
        try:
            # Load existing state first to preserve other data
            existing_state = {}
            if os.path.exists(GAME_STATE_FILE):
                with open(GAME_STATE_FILE, 'r') as f:
                    existing_state = json.load(f)
            
            # Update only energon-related data (excluding player_stats which is now in user_data_manager)
            ...
            temp_file = f"{GAME_STATE_FILE}.tmp"
    ...
    def get_player_stats(self, player_id: str) -> Dict[str, Any]:
        ...
        return user_data_manager.get_energon_stats(player_id)
...
class SlotMachineView(discord.ui.View):
    ...
    def __init__(self, user: discord.Member, mode: str, difficulty: str, bet: int, 
    ...
    async def perform_spin(self, interaction: discord.Interaction):
        """Perform the slot machine spin animation."""
        embed = discord.Embed(
        ...
        spin_duration = 10
        update_interval = 1
        total_updates = int(spin_duration / update_interval)
        ...
        for i in range(total_updates):
            try:
                ...
                progress = (i + 1) / total_updates
                if progress < 0.3:
                    status = "**🎰 SPINNING... 🎰**"
                elif progress < 0.7:
                    status = "**⚡ SPINNING FAST! ⚡**"
                else:
                    status = "**🐢 SLOWING DOWN... 🐢**"
                ...
                remaining_time = spin_duration - (i * update_interval)
                embed.set_footer(text=f"Spinning... {remaining_time:.1f}s remaining", icon_url=self.bot.user.display_avatar.url)
    ...
    async def show_results(self, slot_emojis: List[str]):
        """Show the final results of the slot machine spin."""
        ...
        if self.mode == "bet":
            try:
                ...
                energon_data = await user_data_manager.get_energon_data(str(self.user.id), self.user.display_name)
                new_energon = energon_data.get('energon', 0) + (winnings if winnings > 0 else 0)
                energon_data['energon'] = new_energon
                await user_data_manager.save_energon_data(str(self.user.id), self.user.display_name, energon_data)
        ...
        play_again_view = PlayAgainView(self.user, self.mode, self.difficulty, 
...
class PlayAgainView(discord.ui.View):
    ...
    def __init__(self, user: discord.Member, mode: str, difficulty: str, current_energon: int):
    ...
    async def play_again(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async def difficulty_callback(interaction: discord.Interaction):
            ...
            if self.mode == "bet":
                ...
                await EnergonCommands.start_slot_game(interaction, self.mode, difficulty, 0, self.user, 0)
...
class MarketConfig:
    ...
    # File paths
    CYBERCOIN_MARKET_PATH = os.path.join(os.path.dirname(ENERGON_PETS_DIR), "Global Saves", "cybercoin_market_data.json")
...
class MarketManager:
    ...
    def load_market_data(self) -> Dict:
        ...
        try:
            if os.path.exists(MarketConfig.CYBERCOIN_MARKET_PATH):
                with open(MarketConfig.CYBERCOIN_MARKET_PATH, 'r') as f:
                    data = json.load(f)
    ...
    def save_market_data(self) -> None:
        ...
        try:
            ...
            with open(MarketConfig.CYBERCOIN_MARKET_PATH, 'w') as f:
    ...
    def record_sale(self, user_id: str, coins_sold: float, sale_amount: float, price_per_coin: float) -> dict:
        ...
        return record_cybercoin_sale(user_id, coins_sold, sale_amount, price_per_coin)
...
class CryptoMarketView(discord.ui.View):
    ...
    async def buy_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        current_energon = get_player_energon(player_id)
...
class BuyCoinModal(discord.ui.Modal):
    ...
    def __init__(self, user: discord.Member, current_energon: float, message: discord.Message):
        ...
        self.amount = discord.ui.TextInput(
            label="Amount to invest (Energon)",
            placeholder=f"Min: {MarketConfig.MIN_INVESTMENT}, Max: {current_energon}",
            min_length=1,
            max_length=10
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            invest_amount = float(self.amount.value)
            
            if invest_amount < MarketConfig.MIN_INVESTMENT:
                await interaction.response.send_message(
                    f"❌ Minimum investment is **{MarketConfig.MIN_INVESTMENT} Energon**!",
                    ephemeral=True
                )
                return
            ...
            if invest_amount > self.current_energon:
                await interaction.response.send_message(
                    f"❌ You only have **{self.current_energon} Energon** available!",
                    ephemeral=True
                )
                return
            ...
            coins_received = invest_amount / current_price
            ...
            update_player_energon(player_id, -invest_amount)
            ...
            embed = discord.Embed(
                ...
                description=f"You bought **{coins_received:.4f}** CyberCoins at **{current_price:.2f}** Energon per coin",
...
class SellCoinModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Update player energon
            update_player_energon(player_id, energon_received)
...
def create_market_embed() -> discord.Embed:
    ...
    embed.add_field(
        ...
        value=f"**24h Volume:** {data['total_volume_24h']:.2f} Energon\n"
               f"**Total Coins:** {data['total_coins_in_circulation']:.4f}\n"
    ...
    while not bot.is_closed():
        try:
            ...
            if data['active_event'] and data['event_updates_remaining'] > 0:
                ...
                if holiday and random.random() < MarketConfig.HOLIDAY_EVENT_CHANCE:
                ...
                # Random events
                if random.random() < MarketConfig.EVENT_CHANCE:
...
async def setup(bot_instance: commands.Bot) -> None:
    ...
    # Import and add the EnergonCommands cog
    from Systems.Energon.energon_commands import EnergonCommands
    energon_cog = EnergonCommands(game_manager)
    await bot_instance.add_cog(energon_cog)
    
    print("Energon system initialized successfully!")


# Convenience functions for external use
def save_energon_game_state() -> None:
...
# Energon access functions using user_data_manager
def get_player_energon(player_id: str) -> int:
    ...
    try:
        ...
        energon_data = user_data_manager.get_energon_data(player_id)
        return energon_data.get("energon", 0)
    except Exception:
        return 0

def update_player_energon(player_id: str, amount: int) -> bool:
    ...
    try:
        ...
        energon_data = user_data_manager.get_energon_data(player_id)
        current_energon = energon_data.get("energon", 0)
        new_energon = max(0, current_energon + amount)
        energon_data["energon"] = new_energon
        
        return user_data_manager.save_energon_data(player_id, None, energon_data)
    except Exception:
        return False

# Standalone functions for backwards compatibility
def load_energon_game_state() -> Dict[str, Any]:
