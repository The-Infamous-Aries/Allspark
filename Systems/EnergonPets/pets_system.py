...
class PetSystem:
    ...
    async def load_cyberchronicles_monsters(self) -> None:
        """Legacy method for backward compatibility - delegates to _preload_data"""
        try:
            await self._data_loaded.wait()
            logger.info("Cyberchronicles monsters loaded via legacy method")
        except Exception as e:
            logger.error(f"Error in legacy load method: {e}")
            raise
    ...
    async def charge_pet(self, user_id: int, duration: str) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        """Charge pet's energy"""
        pet = await self.get_user_pet(user_id)
        if not pet:
            return False, "You don't have a pet!", []
        
        duration_map = {"15min": 15, "30min": 30, "1hour": 60}
        minutes = duration_map.get(duration, 15)
        
        max_energy = pet['max_energy'] + (await self.get_equipment_stats(user_id)).get('energy', 0)
        if pet['energy'] >= max_energy:
            return False, f"Your pet is already fully charged! ({int(pet['energy'])}/{max_energy})", []
        
        # Calculate energy gain
        energy_gain = min(minutes * 2, max_energy - pet['energy'])
        pet['energy'] = min(pet['energy'] + energy_gain, max_energy)
        
        # Add XP
        xp_gain = minutes // 5
        leveled_up, level_gains = await add_experience(user_id, xp_gain, "charge")
        
        await self._queue_save(user_id, pet)
        return True, f"Charged your pet for {minutes} minutes! (+{energy_gain}⚡, +{xp_gain}XP)", level_gains

    async def play_with_pet(self, user_id: int, duration: str) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        ...
        max_happiness = pet['max_happiness'] + (await self.get_equipment_stats(user_id)).get('happiness', 0)
        ...
        happiness_gain = min(minutes * 3, max_happiness - pet['happiness'])
        pet['happiness'] = min(pet['happiness'] + happiness_gain, max_happiness)
        
        # Add XP
        xp_gain = minutes // 5
        leveled_up, level_gains = await add_experience(user_id, xp_gain, "play")
        
        await self._queue_save(user_id, pet)
        return True, f"Played with your pet for {minutes} minutes! (+{happiness_gain}😊, +{xp_gain}XP)", level_gains
    ...
    async def train_pet(self, user_id: int, difficulty: str) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        ...
        # Randomly increase attack or defense
        ...
        # Add XP
        xp_gain = 5 * multiplier
        leveled_up, level_gains = await add_experience(user_id, xp_gain, "training")
        
        await self._queue_save(user_id, pet)
        return True, f"Training complete! (+{energon_gain}💰, +{stat_gain} {stat_type}, +{xp_gain}XP)", level_gains

    async def repair_pet(self, user_id: int, duration: str) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        """Repair pet's maintenance"""
        pet = await self.get_user_pet(user_id)
        if not pet:
            return False, "You don't have a pet!", []
        ...
        max_maintenance = pet['max_maintenance'] + (await self.get_equipment_stats(user_id)).get('maintenance', 0)
        if pet['maintenance'] >= max_maintenance:
            return False, f"Your pet is already fully repaired! ({int(pet['maintenance'])}/{max_maintenance})", []
        
        # Calculate maintenance gain
        maintenance_gain = min(minutes * 2, max_maintenance - pet['maintenance'])
        pet['maintenance'] = min(pet['maintenance'] + maintenance_gain, max_maintenance)
        
        # Add XP
        xp_gain = minutes // 5
        leveled_up, level_gains = await add_experience(user_id, xp_gain, "repair")
        
        await self._queue_save(user_id, pet)
        return True, f"Repaired your pet for {minutes} minutes! (+{maintenance_gain}🔧, +{xp_gain}XP)", level_gains
    ...
    async def send_mission(self, user_id: int, difficulty: str) -> Tuple[bool, str]:
        ...
        # Check energy
        energy_cost = 15 * difficulty_level
        if pet['energy'] < energy_cost:
            return False, f"Not enough energy! Need {energy_cost}⚡"
        ...
        # Calculate success
        success_rate = 0.5 + (difficulty_level * 0.1)
        success = random.random() < success_rate
        
        if success:
            energon_gain = random.randint(10, 30) * difficulty_level
            xp_gain = random.randint(5, 15) * difficulty_level
            ...
            leveled_up, level_gains = await add_experience(user_id, xp_gain, "mission")
            
            await self._queue_save(user_id, pet)
            return True, f"Mission successful! (+{energon_gain}💰, +{xp_gain}XP)"
        else:
            return True, "Mission failed! Your pet gained experience but no energon."
    ...
    async def load_transformation_items(self) -> None:
        """Load transformation items (alias for backward compatibility)"""
        await self._data_loaded.wait()

    async def battle_pet(self, user_id: int, opponent_id: int) -> Tuple[bool, str, Dict[str, Any]]:
        """Battle another player's pet"""
        ...
        leveled_up, level_gains = await add_experience(user_id, xp_gain, "battle")
    ...
    async def challenge_pet(self, user_id: int, difficulty: str) -> Tuple[bool, str, Dict[str, Any]]:
        """Challenge a boss monster"""
        pet = await self.get_user_pet(user_id)
        if not pet:
            return False, "You don't have a pet!", {}
        
        difficulty_map = {"easy": 1, "medium": 2, "hard": 3, "extreme": 4}
        difficulty_level = difficulty_map.get(difficulty.lower(), 1)
        ...
        # Calculate pet stats with equipment
        stats = await self.get_equipment_stats(user_id)
        attack = pet['attack'] + stats.get('attack', 0)
        defense = pet['defense'] + stats.get('defense', 0)
        power = attack + defense
        
        # Calculate boss stats based on difficulty
        boss_power = 500 * difficulty_level
        success_rate = min(0.9, power / boss_power)
        
        # Challenge outcome
        energon_gain = 0
        xp_gain = 0
        if random.random() < success_rate:
            energon_gain = random.randint(50, 100) * difficulty_level
            xp_gain = random.randint(25, 50) * difficulty_level
            pet['total_wins'] = pet.get('total_wins', 0) + 1
            pet['total_challenge_energon'] = pet.get('total_challenge_energon', 0) + energon_gain
            result = "win"
            message = f"Challenge successful! You defeated the boss! (+{energon_gain}💰, +{xp_gain}XP)"
        else:
            xp_gain = random.randint(10, 20) * difficulty_level
            pet['total_losses'] = pet.get('total_losses', 0) + 1
            result = "loss"
            message = f"Challenge failed! The boss was too powerful! (+{xp_gain}XP)"
        
        leveled_up, level_gains = await add_experience(user_id, xp_gain, "challenge")
        ...
        return True, message, challenge_result
