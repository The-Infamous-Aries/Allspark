...
class CombinerPvPBattleView(PvPBattleView):
    ...
    async def apply_damage_to_target(self, attacker_id: str, target_id: str, base_damage: int, is_critical: bool = False):
        ...
        try:
            ...
            # Calculate final damage (same as parent method)
            final_damage = base_damage
            if is_critical:
                final_damage = int(final_damage * 1.5)
            
            # Apply defense reduction if target is defending
            if target_id in self.defending_players:
            ...
            # Log the damage
            attacker_name = self.players[attacker_id]['user'].display_name if attacker_id in self.players else "Unknown"
            target_name = target_player['user'].display_name
            damage_type = "💥 Critical" if is_critical else "⚔️"
            
            self.battle_log.append(f"{damage_type} {attacker_name} dealt {final_damage} damage to {target_name}'s combiner team")
...
class PetsMegaCog(commands.Cog):
        ...
        try:
            ...
            for member_id in all_members:
                ...
                if pet_data:
                    ...
                    # Sum up attack and defense stats for battle
                    combined_stats['attack'] += pet_data.get('attack', 10)
                    combined_stats['defense'] += pet_data.get('defense', 5)
                    ...
                    # Calculate health from energy, maintenance, and happiness
                    current_energy = pet_data.get('energy', 100)
                    current_maintenance = pet_data.get('maintenance', 100)
                    current_happiness = pet_data.get('happiness', 100)
                    
                    # Battle health is sum of current energy, maintenance, and happiness
                    pet_battle_health = current_energy + current_maintenance + current_happiness
                    combined_stats['current_health'] += pet_battle_health
                    
                    # Max health is sum of max energy, maintenance, and happiness
                    ...
                    pet_max_health = max_energy + max_maintenance + max_happiness
    ...
    async def update_player_stats(self, user_id, stat_name, value):
        ...
        try:
            user_data = self.bot.user_data_manager.get_user_data(str(user_id))
            if not user_data:
                return
            ...
            if stat_name not in user_data['stats']:
                user_data['stats'][stat_name] = 0
