    ...
    async def process_combat_round(self, monster_action: str):
        ...
        for player_id, action_data in self.player_actions.items():
            ...
            if action_data['action'] == "attack":
                # Use unified damage calculator for player attacks
                damage_result = self.damage_calculator.calculate_attack_damage(
                    attacker=player_data,
                    target=self.monster,
                    charge_multiplier=player_data['charge'],
                    is_defending=False  # Monsters don't defend in PvE
                )
                
                self.monster_hp = max(0, self.monster_hp - damage_result['damage'])
                
                # Track damage dealt by player
                self.total_damage_dealt[player_id] += damage_result['damage']
                self.total_monster_damage_received += damage_result['damage']
                
                # Add action message to battle log
                crit_text = "💥 CRITICAL HIT! " if damage_result['is_critical'] else ""
                charge_text = f" (Charged x{player_data['charge']:.1f}!)" if player_data['charge'] > 1.0 else ""
                action_text = f"{crit_text}{player_data['user'].mention} attacks with a {player_data['charge']:.1f}x charge!{charge_text}"
        ...
        if self.monster_hp > 0:
            ...
            if monster_action == "attack":
                ...
                # Use unified damage calculator for monster attacks
                player_defenses = {}
                for player_id, player_data in self.player_data.items():
                    if player_data['alive']:
                        player_defenses[player_id] = {
                            'defense': player_data['pet'].get('defense', 5),
                            'is_defending': player_id in [d[0] for d in self.defending_players],
                            'defending_target': player_id,  # Simplified for PvE
                            'is_charging': player_data.get('charging', False)
                        }
                
                if player_defenses:
                    damage_results = self.damage_calculator.calculate_monster_damage(
                        monster_attack=monster_attack,
                        player_defenses=player_defenses,
                        attack_multiplier=monster_multiplier
                    )
                    
                    # Apply damage results
                    for player_id, result in damage_results.items():
                        if player_id in self.player_data:
                            player_data = self.player_data[player_id]
                            if player_data['alive']:
                                # Apply damage to player
                                player_data['hp'] = max(0, player_data['hp'] - result['final_damage'])
                                self.total_damage_received[player_id] += result['final_damage']
                                self.total_monster_damage_dealt += result['final_damage']
                                
                                # Apply parry damage to monster
                                if result['parry_damage'] > 0:
                                    self.monster_hp = max(0, self.monster_hp - result['parry_damage'])
                                    self.total_damage_dealt[player_id] += result['parry_damage']
                                    self.total_monster_damage_received += result['parry_damage']
    ...
    async def process_round(self):
        ...
        for player_id, action_data in self.player_actions.items():
            if action_data['action'] == 'attack' and action_data['target']:
                ...
                defender_defending = defender_data['charging']
                attacker_is_charging = attacker_data.get('charging', False)
                damage_result = self.damage_calculator.calculate_attack_damage(
                    attacker=attacker_data,
                    target=defender_data,
                    charge_multiplier=attacker_data['charge'],
                    is_defending=defender_defending,
                    attacker_is_charging=attacker_is_charging
                )
                
                # Apply damage and build action text
                if damage_result['parried']:
                    # Parry successful - reflect damage
                    attacker_data['hp'] = max(0, attacker_data['hp'] - damage_result['damage'])
                    action_text += f"⚔️ **{attacker_data['user'].display_name}** attacks **{defender_data['user'].display_name}**! {damage_result['roll_text']}\n"
                    action_text += f"🛡️ **{defender_data['user'].display_name}** parries! Block: {damage_result['defense_used']} ≥ Attack: {damage_result['attack_damage']}\n"
                    if damage_result['damage'] > 0:
                        action_text += f"⚡ Parry damage: {damage_result['damage']} dealt to {attacker_data['user'].display_name}\n"
                else:
                    # Normal attack or failed parry
                    defender_data['hp'] = max(0, defender_data['hp'] - damage_result['damage'])
                    crit_text = "💥 CRITICAL! " if damage_result['is_critical'] else ""
                    defend_text = f", reduced by {damage_result['defense_used']} defense" if damage_result['defense_reduced'] else ""
                    action_text += f"⚔️ {crit_text}**{attacker_data['user'].display_name}** attacks **{defender_data['user'].display_name}**! {damage_result['roll_text']}{defend_text} → {damage_result['damage']} damage dealt\n"
