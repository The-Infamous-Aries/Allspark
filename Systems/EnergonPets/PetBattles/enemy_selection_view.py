...
class EnemySelectionView(discord.ui.View):
    ...
    def __init__(self, ctx, battle_type="solo"):
        ...
        self.message = None
        self.battle_started = False  # Track if battle was successfully started
    ...
    async def start_battle(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            if self.battle_type == "group":
                ...
                self.battle_started = True  # Mark battle as started
                self.stop()  # Stop the selection view to prevent timeout
            else:
            ...
            self.stop()  # Stop the view on error to prevent timeout message
    ...
    async def cancel_battle(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            self.stop()  # Stop the view when cancelled
        except discord.errors.NotFound:
    ...
    async def on_timeout(self):
        """Handle timeout - only show timeout message if battle wasn't started"""
        try:
            if self.message and not self.battle_started:
                await self.message.edit(
