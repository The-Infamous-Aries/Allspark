...
class EnergonGameManager:
    ...
    async def save_player_energon_to_global(self, player_id: str, energon_amount: int) -> None:
        ...
        try:
            ...
            try:
                user_data = await user_data_manager.get_user_data(player_id, "EnergonPlayer")
    ...
    async def use_energon_for_bet_or_challenge(self, player_id: str, amount: int) -> tuple[bool, str]:
        """
        Use energon for bets/challenges. Checks current energon first, then banked energon.
        ...
        try:
            ...
            if in_game:
                ...
                # Not in game: use banked energon for challenges/bets
...
class MarketManager:
    ...
    def load_market_data(self) -> Dict:
        ...
        try:
            ...
            # Get market data from user_data_manager
            loop = asyncio.get_event_loop()
            if loop.is_running():
                # If already in async context, use the async method
                data = loop.run_until_complete(user_data_manager.get_cybercoin_market_data())
            else:
                # Create new event loop if needed
                new_loop = asyncio.new_event_loop()
                asyncio.set_event_loop(new_loop)
                try:
                    data = new_loop.run_until_complete(user_data_manager.get_cybercoin_market_data())
                finally:
                    new_loop.close()
    ...
    def save_market_data(self) -> None:
        ...
        try:
            ...
            # Save market data using user_data_manager
            loop = asyncio.get_event_loop()
            if loop.is_running():
                loop.run_until_complete(user_data_manager.save_cybercoin_market_data(data_to_save))
            else:
                new_loop = asyncio.new_event_loop()
                asyncio.set_event_loop(new_loop)
                try:
                    new_loop.run_until_complete(user_data_manager.save_cybercoin_market_data(data_to_save))
                finally:
                    new_loop.close()
                    asyncio.set_event_loop(loop)
...
async def update_player_energon(player_id: str, amount: int) -> bool:
    ...
    try:
        ...
        return await user_data_manager.save_energon_data(player_id, energon_data)
    except Exception:
        return False
