...
class RPGCommands(commands.Cog):
    ...
    async def _load_data_async(self, data_type: str) -> Dict[str, Any]:
        ...
        try:
            if data_type == "monsters_and_bosses":
                ...
                return await self.user_data_manager.get_pet_equipment_data()
            elif data_type == "all":
                # Load all available data
                monsters_bosses = await self.user_data_manager.get_monsters_and_bosses_data()
                pet_equipment = await self.user_data_manager.get_pet_equipment_data()
                return {
                    "monsters_and_bosses": monsters_bosses,
    ...
    async def handle_ai_battle(self, channel, players: List[discord.User], pets: List, enemy_type: str = "random"):
        ...
        try:
            # For now, handle single player battles (first player in list)
            if not players or not pets:
                await channel.send("❌ No players or pets available for battle!")
                return
                
            player = players[0]
            pet_data = pets[0] if isinstance(pets[0], dict) else await self.user_data_manager.get_pet_data(str(player.id))
            
            # Create a context object for the battle
            ctx = type('Context', (), {
                'channel': channel,
                'send': channel.send,
                'author': player,
            ...
            # Determine enemy type and rarity
            enemy_rarity = "common"
            if enemy_type == "random":
                enemy_type = random.choice(["monster", "boss"])
                enemy_rarity = random.choice(["common", "rare", "epic"])
            
            # Use the rpg_system's start_battle_encounter method
            battle_view = await self.rpg_system.start_battle_encounter(
                str(player.id), 
                ctx, 
                enemy_type, 
                enemy_rarity
            ...
            if not battle_view:
                await channel.send("❌ Failed to create battle encounter!")
                return
            
            # Create battle embed with AI-generated encounter text
            encounter_text = getattr(battle_view, 'encounter_description', 'A wild enemy appears!')
            
            embed = discord.Embed(
                title=f"⚔️ Battle Encounter",
                description=encounter_text,
                color=discord.Color.red()
            )
            
            # Show party info
            pet_name = pet_data.get('name', 'Unknown') if pet_data else 'Unknown'
            pet_level = pet_data.get('level', 1) if pet_data else 1
            embed.add_field(
                name="🤖 Your Pet", 
                value=f"👤 {player.display_name} ({pet_name} - Lv.{pet_level})", 
                inline=False
            )
...
3. [Choice 3]"""
    ...
    async def start_battle(self, channel, players: List[discord.User], rarity: str, battle_type: str):
        ...
        try:
            ...
            # Load data asynchronously and get enemy
            if battle_type == "Monster":
                enemy_data = await self.rpg_system.get_monster_by_rarity("monsters", rarity.lower())
            elif battle_type == "Boss":
                enemy_data = await self.rpg_system.get_monster_by_rarity("bosses", rarity.lower())
            elif battle_type == "Titan":
                enemy_data = await self.rpg_system.get_titan_for_fight()
            
            if not enemy_data:
                await channel.send(f"❌ Could not find a {battle_type.lower()} of {rarity} rarity!")
                return
            
            # Use the existing battle system for each player
            for player in players:
                try:
                    # Create a mock context for the battle system
                    class MockContext:
                    ...
                    mock_ctx = MockContext(channel, player)
                    
                    # Start battle using the existing RPG system
                    battle_view = await self.rpg_system.start_battle_encounter(
                        str(player.id), mock_ctx, battle_type.lower(), rarity.lower()
                    )
                    
                    if battle_view:
                        embed = discord.Embed(
                            title=f"⚔️ {player.display_name} vs {enemy_data['name']}",
                            description=f"Battle initiated for {player.display_name}!",
                            color=discord.Color.red()
                        )
                        await channel.send(embed=embed)
                    
                except Exception as e:
                    logger.error(f"Error starting battle for {player.display_name}: {e}")
                    await channel.send(f"❌ Failed to start battle for {player.display_name}")
                    
        except Exception as e:
            logger.error(f"Error in start_battle: {e}")
    ...
    async def handle_first_react_event(self, channel, players: List[discord.User], event: Dict):
        ...
        try:
            ...
            embed = discord.Embed(
                title="⏰ Event Timeout",
                description="The event has timed out. No choice was made.",
                color=discord.Color.red()
            )
            await channel.send(embed=embed)
            return
    ...
    async def handle_story_segment(self, channel, players: List[discord.User], story: Dict):
        ...
        if story.get('choices'):
            ...
            # Wait for voting with timeout (non-blocking for other commands)
            try:
                await asyncio.wait_for(view.wait(), timeout=300.0)  # 5 minute timeout
            except asyncio.TimeoutError:
                # Handle timeout gracefully
                embed = discord.Embed(
                    ...
                    description="The voting has timed out. No choice was made.",
                    color=discord.Color.red()
                )
                await channel.send(embed=embed)
                return
    ...
    async def cyber_info(self, ctx):
        ...
        embed = discord.Embed(
            ...
            description="Welcome to the AI-powered Transformers RPG! Experience dynamic adventures with Google Gemini AI storytelling and immersive CyberChronicles campaigns.",
            color=discord.Color.blue()
        )
        
        embed.add_field(
            name="🎯 Getting Started",
            value="Create your pet using the EnergonPets system, then embark on AI-generated adventures! Each pet's level, equipment, and faction affect their effectiveness in the mechanical world of Cybertron.",
            inline=False
        )
        
        embed.add_field(
            name="⚡ Resource System",
            value="Pets have three vital resources that affect all adventures:\n🔋 **Energy** - Physical activities, combat, and exploration\n🔧 **Maintenance** - Repairs, technical challenges, and system interfaces\n😊 **Happiness** - Social interactions, diplomacy, and morale",
            inline=False
        )
        
        embed.add_field(
            name="🌟 CyberChronicles Adventures",
            value="**`/start_cyberchronicles <pet>`** - Begin an immersive solo AI adventure with persistent story continuity\n**`/stop_cyberchronicles`** - End your current CyberChronicles session\n\nExperience connected storylines where your choices shape an evolving narrative!",
            inline=False
        )
        
        embed.add_field(
            name="🎲 Group Adventure Commands",
            value="**`/cyber_random`** - AI chooses adventure type for your party\n**`/cyber_battle`** - Combat-focused encounters with tactical challenges\n**`/cyber_event`** - Resource-based challenges and puzzles\n**`/cyber_story`** - Narrative-driven story segments with meaningful choices",
            inline=False
        )
        
        embed.add_field(
            name="🧠 Advanced AI Features",
            value="• **Dynamic Events** - AI generates unique scenarios based on your pet's stats and history\n• **Story Continuity** - AI remembers past adventures and builds connected narratives\n• **Adaptive Difficulty** - Challenges scale with your effective level (base + equipment bonuses)\n• **Contextual Storytelling** - Events reference previous outcomes and character development",
            inline=False
        )
        
        embed.add_field(
            name="⚔️ Combat & Rewards",
            value="• **AI Battle Encounters** - Dynamic combat with tactical descriptions\n• **XP & Loot Scaling** - Rewards based on effective level and performance\n• **Equipment Bonuses** - Gear provides stat increases that affect success rates\n• **Story Moments** - Key events are tracked for narrative continuity",
            inline=False
        )
        
        embed.add_field(
            name="🏆 Progression System",
            value="Earn XP, discover loot, and manage resources while building your unique story. Equipment bonuses increase your effective level, improving success chances and reward quality. Every adventure contributes to your pet's growing legend!",
            inline=False
        )
        
        embed.set_footer(text="🚀 Form parties or go solo - let the AI craft your epic Transformers adventure!")
    ...
    async def start_cyberchronicles(self, interaction: discord.Interaction, pet: str):
        ...
        user_pets = await self.rpg_system.get_user_pets(user_id, username)
        selected_pet = None
        for p in user_pets:
    ...
    async def load_pet_equipment_async(self):
        """Load pet equipment data using user_data_manager"""
        data = await self._load_data_async("pet_equipment")
        self.pet_equipment = data or {}
    ...
    async def start_battle(self, channel, players, rarity, enemy_type):
        ...
        try:
            ...
            if enemy_type.lower() == "monster":
                enemy_data = await self.rpg_system.get_monster_by_rarity("monster", rarity)
            elif enemy_type.lower() == "boss":
                enemy_data = await self.rpg_system.get_monster_by_rarity("boss", rarity)
            ...
            # Use the existing battle system for group battles
            # For now, start individual battles for each player
            for player in players:
                try:
                    # Create a mock context for the battle system
                    class MockContext:
                    ...
                    # Start battle using the existing RPG system
                    battle_view = await self.rpg_system.start_battle_encounter(
                        str(player.id), mock_ctx, enemy_type.lower(), rarity
                    )
            ...
            logger.error(f"Error in start_battle: {e}")
            await channel.send("❌ Failed to start group battle!")
...
class AIEventChoiceView(discord.ui.View):
    ...
    async def pay_cost_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            # Process resource cost payment for the specific user
            result = await self.ai_system.handle_event_choice(
                str(interaction.user.id), self.event.id, 'accept', interaction
            )
            ...
            if result['success']:
                if result['outcome'] == 'success':
                    ...
                    cost_text = f"Energy: -{self.event.resource_costs.energy}\n"
                    cost_text += f"Maintenance: -{self.event.resource_costs.maintenance}\n"
                    cost_text += f"Happiness: -{self.event.resource_costs.happiness}"
                    ...
                    if result.get('loot'):
                        ...
                        result_embed.add_field(name="🎁 Loot Gained", value=loot_text, inline=False)
                    
                    # Show level up if occurred
                    if result.get('level_up'):
                        result_embed.add_field(name="🎖️ Level Up!", value="Your pet gained a level!", inline=False)
                    
                elif result['outcome'] == 'failure':
                    # Failed event, trigger battle
                    result_embed = discord.Embed(
                        title="⚔️ Event Failed - Battle Triggered!",
                        description=result['message'],
                        color=discord.Color.orange()
                    )
                    ...
                    await self.rpg_commands.handle_ai_battle(
                        interaction.channel, [interaction.user], [await self.rpg_commands.user_data_manager.get_pet_data(str(interaction.user.id))], "random"
                    )
                    return
            else:
                # Not enough resources or other error
                result_embed = discord.Embed(
                    title="❌ Cannot Pay Cost",
                    description=result['message'],
                    color=discord.Color.red()
                )
                ...
                if 'needed' in result:
                    ...
                    cost_text = f"Energy: {needed['energy']}\n"
                    cost_text += f"Maintenance: {needed['maintenance']}\n"
                    cost_text += f"Happiness: {needed['happiness']}"
                    result_embed.add_field(name="💰 Resources Needed", value=cost_text, inline=False)
    ...
    async def refuse_fight_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            result = await self.ai_system.handle_event_choice(
                str(interaction.user.id), self.event.id, 'refuse', interaction
            )
