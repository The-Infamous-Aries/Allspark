...
class UserDataManager:
    ...
    def __init__(self, data_directory: str = "Systems/Users"):
        ...
        self._critical_files = {'pet_equipment', 'monsters', 'bosses', 'titans', 'pets_level', 'energon_game', 'cybercoin_market_data'}
    ...
    async def update_walk_tru_progress(self, user_id: str, username: str, adventure_type: str, progress_data: Dict[str, Any]) -> bool:
        """Update user's Walk Tru adventure progress"""
        try:
            user_data = await self.get_user_data(user_id, username)
            
            if "walk_tru_progress" not in user_data:
                user_data["walk_tru_progress"] = {}
            
            user_data["walk_tru_progress"][adventure_type] = progress_data
            
            return await self.save_user_data(user_id, username, user_data)
        except Exception as e:
            logging.error(f"Error updating Walk Tru progress for {user_id}: {e}")
            return False

    async def get_walk_tru_progress(self, user_id: str, username: str = None, adventure_type: str = None) -> Dict[str, Any]:
        """Get user's Walk Tru adventure progress"""
        try:
            user_data = await self.get_user_data(user_id, username)
            progress = user_data.get("walk_tru_progress", {})
            
            if adventure_type:
                return progress.get(adventure_type, {})
            
            return progress
        except Exception as e:
            logging.error(f"Error getting Walk Tru progress for {user_id}: {e}")
            return {}
