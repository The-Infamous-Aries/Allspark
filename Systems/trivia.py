...
class TriviaView(discord.ui.View):
    ...
    async def handle_answer(self, interaction: discord.Interaction, choice: str):
        ...
        if is_correct:
            ...
            # Disable all buttons since someone got it right
...
class Trivia(commands.Cog):
    ...
    async def get_trivia_questions(self, category: str) -> List[Dict]:
        ...
        if category in method_map:
            return await method_map[category]()
        return []
    ...
    async def show_question(self, session: TriviaSession):
        ...
        if len(options) >= 4:
            embed.add_field(name="🅰️ A", value=options[0], inline=True)
            embed.add_field(name="🅱️ B", value=options[1], inline=True)
            embed.add_field(name="🅲 C", value=options[2], inline=True)
            embed.add_field(name="🅳 D", value=options[3], inline=True)
