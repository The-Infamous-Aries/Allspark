...
class StressTestView(discord.ui.View):
    ...
    async def run_stress_test(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Real bot commands to test based on README.md
            test_commands = [
                # Core bot commands
                "ping", "features", "hello", 
                
                # EnergonPets system commands
                "pet", "energon_stats", "battle_info", "cybercoin_market",
                "pet_equipment", "battle_stats", 
                
                # RPG system commands  
                "character_view", "cyber_info",
                
                # Random system commands
                "range", "rangestats", "joke", "user_says", "what_is",
                "blessing", "lore_stats", "profile", "analysis",
                
                # Fun commands
                "walktru", "combiner", "mega_fight"
            ]
                ...
                while self.is_running and (time.time() - start_time) < self.test_duration:
                    ...
                    for user in test_users:
                        ...
                        for _ in range(self.commands_per_user):
                            ...
                            # Check timeout again for immediate stop
                            if (time.time() - start_time) >= self.test_duration:
                                self.is_running = False
                                break
                            
                            # Select command
                            command_name = test_commands[commands_executed % len(test_commands)]
                            ...
                            try:
                                ...
                                if command and command.is_on_cooldown(user):
                                    # Skip if on cooldown
                                    await asyncio.sleep(0.5)
                                    continue
                                
                                # Execute real command with mock context
                                if command:
                                    # Create mock context
                                    mock_ctx = type('MockContext', (), {
                                        ...
                                        'reply': lambda *args, **kwargs: interaction.channel.send(*args, **kwargs),
                                        'defer': lambda: asyncio.sleep(0),
                                        'respond': lambda *args, **kwargs: interaction.channel.send(*args, **kwargs),
                                        'followup': type('MockFollowup', (), {
                                            'send': lambda *args, **kwargs: interaction.channel.send(*args, **kwargs)
                                        })()
                                    })()
                                    ...
                                    try:
                                        ...
                                        # Log command errors but continue test
                                        pass
                                
                            except Exception as e:
                                # Continue on any command error
                                pass
                            ...
                            await progress_msg.edit(
                                content=f"🔥 Stress Test Running (Real Commands)...\n"
                            ...
                            commands_executed += 1
                            await asyncio.sleep(0.2)  # Realistic delay between commands
            except asyncio.CancelledError:
                # Handle cancellation from stop button
                self.is_running = False
            
            # Test complete - determine if it was stopped or timed out
            self.is_running = False
            stats = monitor.get_system_info()
            ...
            if elapsed_time >= self.test_duration:
                status = "Complete"
                completion_message = "✅ Stress test completed successfully!"
            else:
                status = "Stopped"
                completion_message = "🛑 Stress test stopped by user!"
            
            final_embed = self.create_stress_embed(status, {
                'bot_memory_mb': stats['bot_memory_mb'],
                'bot_cpu_percent': stats['bot_cpu_percent'],
                'commands_per_second': commands_executed / max(1, elapsed_time)
            })
            
            await progress_msg.edit(
                content=f"{completion_message}\n"
                       f"📊 **Final Results (Real Commands):**\n"
                       f"- Real commands executed: {commands_executed}\n"
                       f"- Peak memory: {stats['bot_memory_mb']:.1f}MB\n"
                       f"- Final CPU: {stats['bot_cpu_percent']:.1f}%\n"
                       f"- Average commands/sec: {commands_executed/max(1, elapsed_time):.1f}\n"
                       f"- Test users simulated: {self.test_users}\n"
                       f"- Commands per user: {self.commands_per_user}",
                embed=None
            )
