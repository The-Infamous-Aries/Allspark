...
class RecruitmentTracker:
    ...
    def __init__(self, data_dir: str):
        ...
        self.tracking_file = os.path.join(data_dir, 'recruitment_history.json')
        self.history = self._load_history()
    
    def _load_history(self) -> Dict:
        """Load recruitment history from JSON file."""
        try:
            ...
            print(f"Error loading recruitment history: {e}")
            return {}
    
    def _save_history(self) -> None:
        """Save recruitment history to JSON file."""
        try:
            os.makedirs(os.path.dirname(self.tracking_file), exist_ok=True)
            with open(self.tracking_file, 'w', encoding='utf-8') as f:
                json.dump(self.history, f, indent=2, default=str)
        except Exception as e:
            print(f"Error saving recruitment history: {e}")
    
    def record_message_sent(self, nation_id: str, message_number: int, leader_name: str = None) -> None:
    ...
    def can_send_message(self, nation_id: str, message_number: int) -> bool:
        """
    ...
    def get_available_messages(self, nation_id: str, total_messages: int) -> List[int]:
        """Get list of available message numbers that can be sent to a nation."""
        available = []
        
        for message_num in range(1, total_messages + 1):
            if self.can_send_message(nation_id, message_num):
                available.append(message_num)
        
        return available
    
    def get_cooldown_info(self, nation_id: str) -> Dict:
    ...
    def get_recruitment_stats(self) -> Dict:
        ...
        for nation_id, data in self.history.items():
            cooldown_info = self.get_cooldown_info(nation_id)
        ...
        return {
            ...
            'tracking_file': os.path.basename(self.tracking_file)
        }
