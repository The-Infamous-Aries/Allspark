...
class RecruitPaginatorView(discord.ui.View):
    ...
    async def mass_recruit_all_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.followup.send(f"🚀 Starting MASS recruitment for {total_nations} total nations... This will take approximately {total_nations} seconds due to rate limiting.", ephemeral=True)
        ...
        for i, nation in enumerate(self.nations):
            try:
                ...
                # Rate limiting - wait 1 second between messages to avoid API overload
                if i % 10 == 0:  # Send progress update every 10 messages
                    await interaction.followup.send(f"📊 Progress: {i+1}/{total_nations} processed (Sent: {sent_count}, Failed: {failed_count})", ephemeral=True)
        ...
        await interaction.followup.send(f"✅ MASS recruitment complete! Total: {total_nations}, Sent: {sent_count}, Failed: {failed_count}", ephemeral=True)
        ...
        await interaction.followup.send(
            f"📬 **Recruitment Results**\n"
            f"✅ Successfully sent: **{success_count}**\n"
            f"❌ Failed to send: **{failed_count}**\n"
            f"\n*Failures may be due to API limits, nation settings, or temporary issues.*",
            ephemeral=True
        )
