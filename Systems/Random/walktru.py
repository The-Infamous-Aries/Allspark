...
class StoryMapManager:
    ...
    def __init__(self, bot):
        ...
        self.walk_tru_path = Path("Systems/Random/Walk Tru")
    ...
    async def load_story_maps(self):
        ...
        # Get all JSON files from the story maps directory
        story_map_dir = Path("Systems/Random/Walk Tru")
        
        if not story_map_dir.exists():
            logger.warning("Story maps directory not found")
            return self.story_maps
        
        # Define story map configurations
        story_configs = {
            'horror': {
                'file': 'Horror.json',
        ...
        for adventure_type, config in story_configs.items():
            file_path = story_map_dir / config['file']
            try:
                ...
                logger.warning(f"Story map file not found: {file_path}")
            except json.JSONDecodeError as e:
                logger.error(f"Error parsing JSON file {file_path}: {e}")
            except Exception as e:
                logger.error(f"Error loading story map {file_path}: {e}")
    ...
    async def _load_story_data(self, story_key):
        """Helper method to load story data directly from JSON files"""
        try:
            if story_key not in self._story_configs:
                self.logger.error(f"Unknown story key: {story_key}")
                return None
                
            filename = self._story_configs[story_key]['filename']
            file_path = self.walk_tru_path / filename
            
            if not file_path.exists():
                self.logger.error(f"Story file not found: {file_path}")
                return None
            ...
            if not story_data:
                self.logger.error(f"Story data empty for {filename}")
                return None
