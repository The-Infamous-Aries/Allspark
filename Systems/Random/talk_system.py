...
# Import from config (assuming these exist)
try:
...
# =============================================================================
# SECTION 1: JSON DATA MANAGEMENT
# =============================================================================

def load_json_data(filename: str, default: Any = None) -> Any:
    """Safely load JSON data from Talk folder using absolute path"""
    try:
        # Get the absolute path to the Talk directory relative to this file
        current_dir = os.path.dirname(os.path.abspath(__file__))
        talk_dir = os.path.join(current_dir, "Talk")
...
def save_json_data(filename: str, data: Any) -> bool:
    """Safely save JSON data to Talk folder using relative path"""
    try:
        talk_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Talk")
...
class LoreSystem:
    ...
    def load_lore_data(self):
        """Load lore data from JSON"""
        self.lore_data = load_json_data(self.lore_file, {})
        self._lore_loaded = True
    
    def save_lore_data(self):
        """Save lore data to JSON"""
        self._ensure_lore_loaded()
        save_json_data(self.lore_file, self.lore_data)
    
    def add_lore_entry(self, title: str, description: str, author_id: int, timestamp: datetime) -> bool:
    ...
    def get_lore_entry(self, title: str) -> Optional[Dict]:
        """Get a specific lore entry"""
        self._ensure_lore_loaded()
        return self.lore_data.get(title)
    
    def get_all_lore_titles(self) -> List[str]:
        """Get all lore entry titles"""
        self._ensure_lore_loaded()
        return list(self.lore_data.keys())
    
    def get_random_lore(self) -> Optional[Dict]:
    ...
    def has_lore_entries(self) -> bool:
        """Check if there are any lore entries"""
        self._ensure_lore_loaded()
        return len(self.lore_data) > 0

# =============================================================================
# SECTION 3: TRANSFORMERS LORE LOOKUP
# =============================================================================
...
class TransformersLore:
    ...
    def _ensure_lore_loaded(self):
        ...
        if not self._lore_loaded:
            self.lore_data = load_json_data("what.json", {})
            self._lore_loaded = True

    def _ensure_blessings_loaded(self):
        """Ensure blessings data is loaded before access"""
        if not self._blessings_loaded:
            self.blessings_data = load_json_data("blessings.json", {})
            self._blessings_loaded = True
    
    def get_lore_entry(self, topic: str) -> Optional[Dict]:
    ...
    def get_blessing(self, category: Optional[str] = None) -> Optional[Tuple[str, str]]:
        """Get a blessing from the Allspark"""
        self._ensure_blessings_loaded()
...
class JokeSystem:
    ...
    def get_random_joke(self, category: str = None) -> Dict:
        ...
        jokes_data = load_json_data('Json/jokes.json', {})
        transformers_jokes = jokes_data.get('transformers_politics_war_jokes', [])
        ...
        # For any other category (dark, pun, spooky, christmas), return None
        # This will force the calling function to use the API for these categories
        return None

# =============================================================================
# SECTION 5: USER ANALYSIS SYSTEM
# =============================================================================
...
class UserAnalysis:
    ...
    def __init__(self):
        ...
        templates_data = load_json_data('Json/talk_templates.json', {})
    ...
    def generate_dialogue(self, user_words: Dict[discord.User, List[str]]) -> str:
...
class LorePaginator(View):
    """Paginated view for browsing lore entries"""
    ...
    async def setup(self):
        ...
        self.titles = self.lore_system.get_all_lore_titles()
        self.total_pages = (len(self.titles) + self.entries_per_page - 1) // self.entries_per_page
    ...
    async def create_embed(self, page: int) -> discord.Embed:
        ...
        end_idx = min(start_idx + self.entries_per_page, len(self.titles))
        page_titles = self.titles[start_idx:end_idx]
        ...
        for i, title in enumerate(page_titles, start=start_idx + 1):
            entry = self.lore_system.get_lore_entry(title)
            ...
            embed.add_field(
                name=f"📜 {i}. {title}",
                value=f"{description}\n\n*— {author_name} | {entry['timestamp'][:19]}*",
                inline=False
            )
        
        embed.set_footer(text=f"Page {page + 1} of {self.total_pages} • {len(self.titles)} total entries")
        return embed
    ...
    @discord.ui.button(label='🔍 View Full Entry', style=discord.ButtonStyle.primary)
    async def view_full_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Show a dropdown to select a specific entry"""
        ...
        for i, title in enumerate(page_titles):
            options.append(discord.SelectOption(
                label=f"{start_idx + i + 1}. {title[:80]}",
                description=f"View full entry: {title[:50]}...",
                value=str(start_idx + i)
            ))
        
        class FullEntrySelect(Select):
            def __init__(self, lore_system, bot):
                super().__init__(placeholder="Choose an entry to view in full...", options=options)
                self.lore_system = lore_system
                self.bot = bot
            
            async def callback(self, select_interaction):
                index = int(self.values[0])
                title = self.view.titles[index]
                entry = self.lore_system.get_lore_entry(title)
                ...
                full_embed.set_footer(text=f"Entry #{index + 1} • Authored by {author_name} | {entry['timestamp'][:19]}")
                await select_interaction.response.send_message(embed=full_embed, ephemeral=True)
        
        class FullEntryView(View):
            def __init__(self, lore_system, bot):
                super().__init__(timeout=60)
                self.add_item(FullEntrySelect(lore_system, bot))
        
        await interaction.response.send_message(
            "Select an entry to view in full detail:", 
            view=FullEntryView(self.lore_system, self.bot), 
            ephemeral=True
        )
    ...
    async def stats_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Show lore collection statistics"""
        ...
        stats_embed = discord.Embed(
            title="📊 Lore Collection Statistics",
            color=discord.Color.green()
        )
        stats_embed.add_field(name="📚 Total Entries", value=str(total_entries), inline=True)
        stats_embed.add_field(name="👥 Contributors", value=str(len(author_counts)), inline=True)
        stats_embed.add_field(name="🏆 Top Contributors", value=contributor_text or "None", inline=False)
...
class TalkSystem(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.transformers_lore = TransformersLore()
        self.joke_system = JokeSystem()
    ...
    async def add_lore(self, ctx, *, title: str = None):
        ...
        if self.lore_system.get_lore_entry(title):
        ...
        try:
            ...
            success = self.lore_system.add_lore_entry(title, description, ctx.author.id, ctx.message.created_at)
    ...
    async def view_lore(self, ctx):
        ...
        if not self.lore_system.has_lore_entries():
            embed = discord.Embed(
    ...
    async def random_lore(self, ctx):
        ...
        entry = self.lore_system.get_random_lore()
    ...
    async def what_is(self, ctx, topic: str):
        ...
        entry = self.transformers_lore.get_lore_entry(topic)
        ...
        if entry:
            ...
            available_topics = ", ".join(self.transformers_lore.get_available_topics())
            embed = discord.Embed(
    ...
    async def what_is_autocomplete(self, interaction: discord.Interaction, current: str):
        ...
        topics = self.transformers_lore.get_available_topics()
        return [
    ...
    async def blessing(self, ctx, category: str = None):
        """Get a blessing from the Allspark"""
        blessing_data = self.transformers_lore.get_blessing(category)
    ...
    async def blessing_autocomplete(self, interaction: discord.Interaction, current: str):
        ...
        categories = self.transformers_lore.get_blessing_categories()
        return [
    ...
    async def joke(self, ctx: commands.Context, category: str = None):
        ...
        async with ctx.typing():
            ...
            if category == "transformers_pnw":
                joke_data = self.joke_system.get_random_joke(category)
            else:
    ...
    async def user_says(self, ctx, members: commands.Greedy[discord.Member] = None):
        ...
        dialogue = self.generate_multi_dialogue(members, user_words)
        ...
        view = UserSaysView(self.bot, ctx.author, display_words, dialogue)
    ...
    async def grump_command(self, ctx):
        ...
        # Check if user has any cybertronian role
        user_roles = [role.id for role in ctx.author.roles]
        cybertronian_roles = list(ROLE_IDS.values())
        
        if not any(role_id in user_roles for role_id in cybertronian_roles):
            await ctx.send("❌ Only Cybertronians can use this command! You need one of the cybertronian roles.")
            return
        ...
        grump_data = load_json_data('Json/grump.json')
        greeting_templates = []
        ...
        selected_greetings = random.sample(greeting_templates, 5)
        
        await ctx.send(f"🧌 {ctx.author.mention} is getting sweet revenge on {grump_mention} 🦹!")
        
        for greeting in selected_greetings:
            await ctx.send(greeting)
        
        await ctx.send(f"We did it! 😂😂😂🤣🤣🤣")
    ...
    async def hello(self, ctx):
        ...
        if user_id in self.bot.hello_cooldowns:
            ...
            if time_diff < 120:  # Within 120 seconds
                ...
                # Get all members in the guild except the disobedient user and the bot
                ...
                if bow_level <= len(bowing_messages):
                    ...
                    if guild_members:
                        ...
                        try:
                            with open('Json/grump.json', 'r', encoding='utf-8') as f:
                                grump_data = json.load(f)
                ...
                bot.hello_cooldowns[user_id] = (current_time, 1)
