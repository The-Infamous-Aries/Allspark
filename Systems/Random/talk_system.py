...
class LoreSystem:
    ...
    async def load_lore_data(self):
        """Load lore data from JSON using OptimizedUserDataManager"""
        try:
    ...
    async def _ensure_templates_loaded(self):
        ...
        if not self._templates_loaded:
            try:
                ...
                # Fallback to direct file access
                templates_data = load_json_data('talk_templates.json', {})
                self.dialogue_templates = templates_data.get('dialogue_templates', [
                    "My Autobot sensors detect a high probability of you saying, 'I need more {user1word1} for my {user1word2} and {user1word3}.'",
                    "Optimus Prime would be proud of your dedication to {user1word1}, {user1word2}, and {user1word3}."
                ])
...
class TalkSystem(commands.Cog):
    ...
    async def grump_command(self, ctx):
        ...
        # Create a single embed with all greetings to avoid rate limits
        embed = discord.Embed(
            title="🧌 Sweet Revenge Time! 🦹",
            description=f"{ctx.author.mention} is getting sweet revenge on {grump_mention}!",
            color=discord.Color.red()
        )
        
        # Add greetings as fields in the embed
        for i, greeting in enumerate(selected_greetings, 1):
            embed.add_field(name=f"Message {i}", value=greeting, inline=False)
        
        embed.set_footer(text="We did it! 😂")
    ...
    async def hello(self, ctx):
        ...
        if user_id in self.bot.hello_cooldowns:
            ...
            if time_diff < 120:  # Within 120 seconds
                ...
                # Get all members in the guild except the disobedient user (including bots)
        ...
        if user_id in self.bot.hello_cooldowns:
            ...
            if time_diff < 120 and bow_level >= 1:  # User is in bowing escalation
                ...
                if any(keyword in message_lower for keyword in bow_keywords):
                    ...
                    # Get all members in the guild except the disobedient user (includes bots for mentions)
