...
class UserDataManager:
    ...
    def __init__(self, data_directory: str = "Systems/Users"):
        ...
        self.json_files = {
            ...
            'energon_game': self.json_path / "energon_game.json",
            'cybercoin_market_data': self.json_path / "cybercoin_market_data.json",
            'roasts': self.json_path / "roasts.json",
            'trivia_transformers_culture': Path("Systems/Data/Trivia/transformers_culture.json"),
            'trivia_transformers_characters': Path("Systems/Data/Trivia/transformers_characters.json"),
            'trivia_transformers_factions': Path("Systems/Data/Trivia/transformers_factions.json"),
            'trivia_transformers_movies': Path("Systems/Data/Trivia/transformers_movies.json"),
            'trivia_transformers_shows': Path("Systems/Data/Trivia/transformers_shows.json")
        }
        ...
        self._file_paths = {
            ...
            'alliance_cache': self.json_path / "alliance_cache.json",
        ...
        self._special_cache_ttl = {
            'alliance_cache': 3600  # 1 hour expiration for alliance cache
        }
        self._file_locks = {}
            ...
            try:
                ...
                if self._backup_enabled:
                    if self._restore_from_backup(file_path):
                        logging.info(f"Successfully restored {file_path} from backup after save failure")
                        return True
    ...
    async def load_json_data(self, key: str) -> Any:
        ...
        Example keys: 'recruit', 'alliance_cache', etc.
        ...
        try:
            ...
            # Special handling for alliance_cache - check expiration
            if key == 'alliance_cache':
                try:
                    # Load the data directly to check timestamps
                    data = await self._load_json_optimized(file_path, default_data=default_data, lazy=False)
                    if data:
                        # Check if any entries are expired (older than 1 hour)
                        now = time.time()
                        expired_keys = []
                        for cache_key, entry in data.items():
                            if isinstance(entry, dict) and 'timestamp' in entry:
                                if now - entry['timestamp'] > 3600:  # 1 hour expiration
                                    expired_keys.append(cache_key)
                        
                        # Remove expired entries
                        for expired_key in expired_keys:
                            data.pop(expired_key, None)
                        
                        # If we removed any entries, save the updated cache
                        if expired_keys:
                            await self.save_json_data(key, data)
                            logging.info(f"Removed {len(expired_keys)} expired entries from alliance_cache")
                    
                    return data
                except Exception as e:
                    logging.error(f"Error checking alliance_cache expiration: {e}")
    ...
    async def monitor_resource_usage(self) -> Dict[str, Any]:
        """Monitor system resource usage for online hosting"""
        import psutil
        import sys
        ...
        try:
            ...
            resource_stats = {
                ...
                'cache': {
                    ...
                    'memory_estimate': sum(sys.getsizeof(v) for v in self._cache.values()) / 1024 / 1024  # MB
                },
                'file_handles': {
            ...
            warnings = []
            if resource_stats['memory']['percent'] > 80:
                warnings.append("High memory usage detected")
            if resource_stats['cache']['size'] > self._max_cache_size * 0.9:
                warnings.append("Cache near capacity")
            if resource_stats['file_handles']['open_files'] > 100:
                warnings.append("High number of open file handles")
            
            resource_stats['warnings'] = warnings
            ...
            return {
                ...
                'basic_metrics': self._metrics.copy(),
                'cache_size': len(self._cache),
                'timestamp': datetime.now().isoformat()
            }
        except Exception as e:
